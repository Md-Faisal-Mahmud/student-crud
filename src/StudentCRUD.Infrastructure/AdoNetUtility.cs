...
namespace FirstDemo.Infrastructure
{
    public class AdoNetUtility : IAdoNetUtility
    {
        ...
        {
            return new NpgsqlParameter(name, CorrectSqlDateTime(value));
        }

        private DbParameter CreateOutputParameter(string name, DbType dbType)
        {
            var outParam = new NpgsqlParameter(name, CorrectSqlDateTime(dbType));
        ...
        {
            var outParam = new NpgsqlParameter(name, GetDbTypeFromType(type));
            outParam.Direction = ParameterDirection.Output;
            return outParam;
        }
